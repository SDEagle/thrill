...
#pragma once
#ifndef THRILL_CORE_FOLD_TABLE_HEADER
#define THRILL_CORE_FOLD_TABLE_HEADER
...
public:
    ...
    static constexpr FoldTableImpl table_impl_ = FoldTableImpl::PROBING;
    ...
    //! of items delivered in the FoldFunction arbitrary.
...
/*!
 * DefaultFoldConfig with implementation type selection
 */
template <FoldTableImpl table_impl>
class DefaultFoldConfigSelect : public DefaultFoldConfig
{
public:
    //! select the hash table in the reduce phase by enum
    static constexpr FoldTableImpl table_impl_ = table_impl;
};
...
template <typename ValueType, typename Key, typename Value,
          ...
          typename FoldConfig_, typename IndexFunction,
...
public:
    ...
    using TableItem = ValueType;
    using MakeTableItem = FoldMakeTableItem<Key, Value, typename TableItem::second_type>;
    ...
    FoldTable(const FoldTable&) = delete;
    //! non-copyable: delete assignment operator
    FoldTable& operator = (const FoldTable&) = delete;
    ...
    //! Returns the fold_function
    const FoldFunction& fold_function() const { return fold_function_; }
    ...
    template<class T>
    TableItem fold(T&& a, const Value& b) {
        return MakeTableItem::Fold(std::forward<T>(a), b, fold_function_);
    }

    TableItem make(const Key& key, const Value& value) {
        return MakeTableItem::Make(key, value, [](const Key&) { return typename TableItem::second_type(); }, fold_function_);
    }
...
//! Type selection via FoldTableImpl enum
template <FoldTableImpl ImplSelect,
          typename ValueType, typename Key, typename Value,
          typename KeyExtractor, typename FoldFunction,
          typename Emitter,
          typename FoldConfig = DefaultFoldConfig,
